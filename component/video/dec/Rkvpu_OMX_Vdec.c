...
OMX_BOOL Rkvpu_SendInputData(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    if (inputUseBuffer->dataValid == OMX_TRUE) {
        ...
        if (pVideoDec->bFirstFrame == OMX_TRUE) {
            ...
            if (((inputUseBuffer->nFlags & OMX_BUFFERFLAG_EXTRADATA) == OMX_BUFFERFLAG_EXTRADATA)
                ...
                if (pVideoDec->bDRMPlayerMode == OMX_TRUE) {
                    ...
                    if (pVideoDec->bDRMPlayerMode == OMX_TRUE) {
#ifdef AVS80
                        OMX_U32 trueAddress = Rockchip_OSAL_SharedMemory_HandleToSecureAddress(pVideoDec->hSharedMemory, (OMX_HANDLETYPE)extraData, 4194304);
            ...
            if (pVideoDec->bDRMPlayerMode == OMX_TRUE) {
                Rockchip_OSAL_SharedMemory_SecureUnmap(pVideoDec->hSharedMemory, extraData, 4194304);
            }
        ...
        if (pVideoDec->bDRMPlayerMode == OMX_TRUE) {
#ifdef AVS80
            OMX_U32 trueAddress = Rockchip_OSAL_SharedMemory_HandleToSecureAddress(pVideoDec->hSharedMemory, (OMX_HANDLETYPE)pkt.data, 4194304);
        ...
        if (pVideoDec->bDRMPlayerMode == OMX_TRUE) {
            Rockchip_OSAL_SharedMemory_SecureUnmap(pVideoDec->hSharedMemory, pkt.data, 4194304);
        }
        if (pkt.size != 0) {
            goto EXIT;
        }
